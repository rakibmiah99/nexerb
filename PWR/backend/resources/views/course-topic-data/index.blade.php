...
@section('content')
    <div class="p-4">
        <div class="card">
            <x-card-header :can-create="true" :url="route('course_topic_data.create')" name="Course Topic Data" url-name="Create">
                <x-slot:buttons>
                    <form action="{{route('course_topic_data.index')}}" class="d-flex">
                        <div class="me-3">
            ...
            <div class="mt-3">
                ...
                <div class="table-responsive table-paginate mt-2 text-nowrap">
                    <table class="table">
                        ...
                        @foreach ($course_topic_data as $key=>$item)
                            <tr>
                                <td>{{$index++}}</td>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" style="">
                                            <a data-bs-toggle="modal" data-bs-target="#viewModal" class="dropdown-item view-btn" href="javascript:void(0);" url="{{route('course_topic_data.show', $item->id)}}"><i class='bx bx-low-vision me-1'></i>View</a>
                                            <a class="dropdown-item" href="{{route('course_topic_data.edit', $item->id)}}"><i class="bx bx-edit-alt me-1"></i>Edit</a>
{{--                                            <a class="dropdown-item" href="{{route('course_topic.changeStatus', $item->id)}}"><i class='bx bx-checkbox-minus'></i>Active</a>--}}
                                            <a data-bs-toggle="modal" data-bs-target="#deleteModal" url="{{route('course_topic_data.delete', $item->id)}}"  class="dropdown-item delete-btn" href="javascript:void(0);"><i class="bx bx-trash me-1"></i>Delete</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <x-when-table-empty :data-length="$course_topic_data->count()"/>
                </div>


                <div class="px-3 mt-3">
                    {{ $course_topic_data->links() }}
                </div>
            </div>
        </div>
    </div>
