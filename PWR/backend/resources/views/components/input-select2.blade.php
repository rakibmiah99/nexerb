...
@if(!$with_code)
    ...
    <div class="mb-3 {{$mode == "horizontal" ? 'row' : ''}}">
        <label @if($size) style="font-size: 10px" @endif for="{{$id}}" class="{{$label_size}} col-form-label">
            {{$title}}
            @if($is_required)
                <x-required/>
        ...
        <div class="{{$input_size}}">
            <select @if($is_multiple) multiple @endif @if($is_required) required @endif name="{{$name}}" class="form-select {{$size}} select-2" id="{{$id}}">
